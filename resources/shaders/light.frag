...
// 
// //uniform samplerCube in_pMap[NUMPOINT];
// //uniform sampler2D in_dMap[NUMDIR];
// //uniform sampler2D in_sMap[NUMSPOT];
// 
// 
...
// 	vec3 directionList[20] = vec3[] //This is an optimisation for PCF (although it does degrade quality a bit, it saves 44 samples)
// 	(
...
//	//inShadow = ShadowCalculation(FragPosLightSpace, in_dMap[i]); //Calculate how much in-shadow the fragment is	(GRAPHICS UNIT)   
...
//  {
//		//inShadow =  ShadowCubeCalculation(ex_FragPos, in_pLight[i].m_pos, in_pMap[i], in_pLight[i].m_radius); //Calculate how much in-shadow the fragment is
//	
...
//		//inShadow = ShadowCalculation(FragPosLightSpace, in_sMap[i]); //Calculate how much in-shadow the fragment is	 
//	
